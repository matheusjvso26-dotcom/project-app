...
import { Plus, Search, Filter, MoreHorizontal, Mail, Phone, MessageCircle, Building2, ArrowUpDown, Trash2, Edit3 } from 'lucide-react'
...
import { createLeadWithDeal, deleteLeadAction } from './actions'
...
const getStatusBadge = (status: string) => {
    ...
    if (s.includes('ganho') || s.includes('cliente') || s.includes('won') || s.includes('fechado')) return <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-indigo-100 text-indigo-700">{status}</span>
        ...
        <div className="p-8 max-w-7xl mx-auto flex flex-col h-full relative z-10 bg-[#151515]">
            ...
            <div className="bg-[#1c1c1c] rounded-b-2xl shadow-sm overflow-hidden flex-1 flex flex-col border-t-0 rounded-t-none">
                                ...
                                <tr key={contact.id} className="hover:bg-white/5 transition-colors">
                                    ...
                                    <td className="px-6 py-5">
                                        <div className="flex items-center gap-1">
                                            <a href={`https://wa.me/${contact.phone.replace(/\D/g, '')}`} target="_blank" rel="noreferrer" className="p-2.5 text-zinc-400 hover:text-white hover:bg-white/5 rounded-full transition-all" title="Chamar no WhatsApp">
                                                <MessageCircle className="w-4 h-4" />
                                            </a>
                                            <a href={`mailto:${contact.email}`} className="p-2.5 text-zinc-400 hover:text-white hover:bg-white/5 rounded-full transition-all" title="Enviar E-mail">
                                                <Mail className="w-4 h-4" />
                                            </a>
                                            <a href={`tel:${contact.phone.replace(/\D/g, '')}`} className="p-2.5 text-zinc-400 hover:text-white hover:bg-white/5 rounded-full transition-all" title="Ligar">
                                    ...
                                    <td className="px-6 py-5 text-right relative">
                                        <button
                                            ...
                                            className="p-2 text-zinc-400 hover:text-white hover:bg-white/5 rounded-md transition-all"
                                        >
                                        ...
                                        {menuOpenId === contact.id && (
                                            <>
                                                ...
                                                <div className="absolute right-6 top-14 mt-1 w-32 bg-[#1a1a1a] border border-white/10 rounded-md shadow-lg z-20 py-1 flex flex-col items-start overflow-hidden">
                                                    <button
                                                        onClick={() => {
                                                            alert(`Em breve: Tela de edição. \nVocê clicou em Editar: ${contact.name}`);
                                                            setMenuOpenId(null);
                                                        }}
                                                        className="w-full text-left px-4 py-2 text-sm text-zinc-300 hover:bg-white/5 flex items-center gap-2"
                                                    >
                                                        <Edit3 className="w-3.5 h-3.5" /> Editar
                                                    </button>
                                                    <button
                                                        onClick={async () => {
                                                            if (confirm('Tem certeza que deseja excluir este Lead do sistema e do funil correspondente?')) {
                                                                const res = await deleteLeadAction(contact.id);
                                                                if (res.success) {
                ...
                <div className="mt-auto px-6 py-4 border-t border-white/5 bg-[#1a1a1a] flex items-center justify-between text-sm text-zinc-500">
                    <div>Mostrando todos os <strong className="text-white">{contacts.length}</strong> registros cadastrados</div>
                    <div className="flex items-center gap-2 opacity-50 pointer-events-none">
                        <Button variant="outline" size="sm" className="bg-transparent border-none text-zinc-500">Anterior</Button>
                        <Button variant="default" size="sm" className="w-8 h-8 p-0 bg-[#ff7b00] text-white">1</Button>
                        <Button variant="outline" size="sm" className="bg-transparent border-none text-zinc-500">Próximo</Button>
                    </div>
                </div>
            </div>
        </div>
