    ...
    try {
        ...
        if (body.object === 'whatsapp_business_account' && body.entry) {
            for (const _entry of body.entry) {
                const metaBusId = _entry.id
                for (const _change of _entry.changes || []) {
                    const changes = _change.value

                    // Tem mensagens ali dentro? (Lead digitou algo para nós)
                    if (changes.messages && changes.messages.length > 0) {
                        for (const incomingMessage of changes.messages) {
                            const contactInfo = changes.contacts?.find((c: any) => c.wa_id === incomingMessage.from) || changes.contacts?.[0]
                            ...
                            // const metaBusId extraido do loop exterior
                    ...
                    if (changes.statuses && changes.statuses.length > 0) {
                        for (const statusEvent of changes.statuses) {
